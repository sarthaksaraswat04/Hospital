...
app.get("/patient_messages", (req, res) => {
    var t = { pn: req.session.pname, ph: req.session.photo }
    res.render("patient_messages", { data: t });
});
...
app.post('/loginProcess', ed, function (req, res) {
    ...
    if (role === 'Admin') {
        var sq = "SELECT * FROM admin WHERE id='" + id + "' AND pwd='" + password + "'";
...
app.post('/doctorSearch', ed, (req, res) => {
    ...
    con.query(sql, function (err, result) {
        if (err) throw err;
            ...
            req.session.ddepart = result[0].Department;
            var t = { dn: req.session.dname, dp:req.session.ddepart, result: result }
            res.render('patient_finddoctor', { data: t });
        }
    });
});
